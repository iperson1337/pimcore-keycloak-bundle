        ...
        $rootNode
            ->children()
                ...
                ->scalarNode('default_language')
                    ->defaultValue('en')
                    ->info('Язык по умолчанию для новых пользователей')
                ->end()
                ...
                ->booleanNode('single_logout')
                    ->defaultTrue()
                    ->info('При выходе из Pimcore редиректить на end-session Keycloak (Single Logout). Выключите, если вход идёт через Direct Grant — браузерной сессии Keycloak в этом случае нет')
                ->end()
